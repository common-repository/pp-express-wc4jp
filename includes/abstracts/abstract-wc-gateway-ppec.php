...
abstract class WC_Gateway_PPEC extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'PayPal Checkout', 'pp-express-wc4jp' );
		$this->method_description = __( 'Allow customers to conveniently checkout directly with PayPal.', 'pp-express-wc4jp' );
		...
		if ( empty( $_GET['woo-paypal-return'] ) && 'yes' !== $this->get_option( 'use_spb', 'yes' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$this->order_button_text = __( 'Continue to payment', 'pp-express-wc4jp' );
		}
	...
	public function process_payment( $order_id ) {
		...
		if ( $checkout->is_started_from_checkout_page() ) {
			...
			try {
				...
				wc_add_notice( __( 'Sorry, an error occurred while trying to process your payment. Please try again.', 'pp-express-wc4jp' ), 'error' );
			} catch ( PayPal_API_Exception $e ) {
	...
	private function get_certificate_info( $cert_string ) {
		if ( ! strlen( $cert_string ) ) {
			return __( 'No API certificate on file.', 'pp-express-wc4jp' );
		}
		...
		if ( $cert_info ) {
			...
			$expires = __( 'expires on %1$s (%2$s)', 'pp-express-wc4jp' );

			if ( $valid_until < time() ) {
				// Display in red if the cert is already expired
				// Translators: placeholders are a date in local format and its timezone.
				$expires = '<span style="color: red;">' . __( 'expired on %1$s (%2$s)', 'pp-express-wc4jp' ) . '</span>';
			...
			$output = sprintf( __( 'Certificate belongs to API username %1$s; %2$s.', 'pp-express-wc4jp' ), $cert_info['subject']['CN'], $expires );
		} else {
			$output = __( 'The certificate on file is not valid.', 'pp-express-wc4jp' );
		}
	...
	protected function validate_active_credentials() {
		...
		if ( ! $has_username ) {
			$errors[] = __( 'Error: You must enter API username.', 'pp-express-wc4jp' );
		}

		if ( ! $has_password ) {
			$errors[] = __( 'Error: You must enter API password.', 'pp-express-wc4jp' );
		}

		if ( ! $has_credential ) {
			$errors[] = __( 'Error: You must provide API signature or certificate.', 'pp-express-wc4jp' );
		}
		...
		if ( $has_username && $has_password && $has_credential ) {
			if ( is_a( $creds, 'WC_Gateway_PPEC_Client_Credential_Signature' ) ) {
				try {
					...
					if ( ! $payer_id ) {
						$errors[] = __( 'Error: The API credentials you provided are not valid.  Please double-check that you entered them correctly and try again.', 'pp-express-wc4jp' );
					}
				} catch ( PayPal_API_Exception $ex ) {

					$errors[] = __( 'An error occurred while trying to validate your API credentials. Unable to verify that your API credentials are correct.', 'pp-express-wc4jp' );
				}
				...
				if ( false === $cert ) {
					$errors[] = __( 'Error: The API certificate is not valid.', 'pp-express-wc4jp' );
				}
				...
				if ( $valid_until < time() ) {
					$errors[] = __( 'Error: The API certificate has expired.', 'pp-express-wc4jp' );
				} elseif ( $cert_info['subject']['CN'] !== $creds->get_username() ) {
					$errors[] = __( 'Error: The API username does not match the name in the API certificate. Make sure that you have the correct API certificate.', 'pp-express-wc4jp' );
				}
		...
		if ( 'yes' === $settings_array['require_billing'] ) {
			...
			if ( ! $is_account_enabled_for_billing_address ) {
				...
				$errors[] = __( 'The "require billing address" option is not enabled by your account and has been disabled.', 'pp-express-wc4jp' );
			}
		}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( 0 == $amount || null == $amount ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
			return new WP_Error( 'paypal_refund_error', __( 'Refund Error: You need to specify a refund amount.', 'pp-express-wc4jp' ) );
		}
		...
		if ( ! isset( $txn_data['refundable_txns'] ) ) {
			return new WP_Error( 'paypal_refund_error', __( 'Refund Error: Sorry! This is not a refundable transaction.', 'pp-express-wc4jp' ) );
		}
		...
		if ( $total_refundable_amount < $amount ) {
			if ( 0 == $total_refundable_amount ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
				return new WP_Error( 'paypal_refund_error', __( 'Refund Error: All transactions have been fully refunded. There is no amount left to refund', 'pp-express-wc4jp' ) );
			} else {
