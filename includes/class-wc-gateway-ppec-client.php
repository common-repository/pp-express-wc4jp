...
class WC_Gateway_PPEC_Client {
	...
	protected function _validate_request() {
		...
		if ( ! $this->_credential ) {
			throw new Exception( __( 'Missing credential', 'pp-express-wc4jp' ), self::INVALID_CREDENTIAL_ERROR );
		}

		if ( ! is_a( $this->_credential, 'WC_Gateway_PPEC_Client_Credential' ) ) {
			throw new Exception( __( 'Invalid credential object', 'pp-express-wc4jp' ), self::INVALID_CREDENTIAL_ERROR );
		}

		if ( ! in_array( $this->_environment, array( 'live', 'sandbox' ), true ) ) {
			throw new Exception( __( 'Invalid environment', 'pp-express-wc4jp' ), self::INVALID_ENVIRONMENT_ERROR );
		}
	}
	...
	protected function _process_response( $response ) {
		if ( is_wp_error( $response ) ) {
			...
			throw new Exception( sprintf( __( 'An error occurred while trying to connect to PayPal: %s', 'pp-express-wc4jp' ), $response->get_error_message() ), self::REQUEST_ERROR );
		}

		parse_str( wp_remote_retrieve_body( $response ), $result );

		if ( ! array_key_exists( 'ACK', $result ) ) {
			throw new Exception( __( 'Malformed response received from PayPal', 'pp-express-wc4jp' ), self::REQUEST_ERROR );
		}
	...
	protected function _get_billing_agreement_description() {
		...
		$description = sprintf( _x( 'Orders with %s', 'data sent to PayPal', 'pp-express-wc4jp' ), get_bloginfo( 'name' ) );
