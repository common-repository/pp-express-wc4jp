...
class WC_Gateway_PPEC_Checkout_Handler {
	...
	public function endpoint_page_titles( $title ) {
		if ( ! is_admin() && is_main_query() && in_the_loop() && is_page() && is_checkout() && $this->has_active_session() ) {
			$title = __( 'Confirm your PayPal order', 'pp-express-wc4jp' );
	...
	public function paypal_billing_details() {
		...
		?>
		<h3><?php esc_html_e( 'Billing details', 'pp-express-wc4jp' ); ?></h3>
		<ul>
			<?php if ( ! empty( $checkout_details->payer_details->billing_address ) ) : ?>
				<li><strong><?php esc_html_e( 'Address:', 'pp-express-wc4jp' ); ?></strong></br><?php echo WC()->countries->get_formatted_address( $this->get_mapped_billing_address( $checkout_details ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></li>
			<?php elseif ( ! empty( $checkout_details->payer_details->first_name ) && ! empty( $checkout_details->payer_details->last_name ) ) : ?>
				<li><strong><?php esc_html_e( 'Name:', 'pp-express-wc4jp' ); ?></strong> <?php echo esc_html( $checkout_details->payer_details->last_name . ' ' . $checkout_details->payer_details->first_name ); ?></li>
			<?php endif; ?>

			<?php if ( ! empty( $checkout_details->payer_details->email ) ) : ?>
				<li><strong><?php esc_html_e( 'Email:', 'pp-express-wc4jp' ); ?></strong> <?php echo esc_html( $checkout_details->payer_details->email ); ?></li>
			<?php else : ?>
			...
			<?php if ( ! empty( $checkout_details->payer_details->phone_number ) ) : ?>
				<li><strong><?php esc_html_e( 'Phone:', 'pp-express-wc4jp' ); ?></strong> <?php echo esc_html( $checkout_details->payer_details->phone_number ); ?></li>
		...
		if ( ! is_user_logged_in() && $checkout->enable_signup ) {
			...
			if ( $checkout->enable_guest_checkout ) {
				...
				<p class="form-row form-row-wide create-account">
					<input class="input-checkbox" id="createaccount" <?php checked( ( true === $checkout->get_value( 'createaccount' ) || ( true === apply_filters( 'woocommerce_create_account_default_checked', false ) ) ), true ); ?> type="checkbox" name="createaccount" value="1" /> <label for="createaccount" class="checkbox"><?php esc_html_e( 'Create an account?', 'pp-express-wc4jp' ); ?></label>
				</p>
				<?php
			}
			...
			if ( ! empty( $checkout->checkout_fields['account'] ) ) {
				...
				<div class="create-account">
					...
					<p><?php esc_html_e( 'Create an account by entering the information below. If you are a returning customer please login at the top of the page.', 'pp-express-wc4jp' ); ?></p>
	...
	public function paypal_shipping_details() {
		...
		?>
		<h3><?php esc_html_e( 'Shipping details', 'pp-express-wc4jp' ); ?></h3>
		<?php
	...
	public function maybe_return_from_paypal() {
		...
		if ( empty( $session ) || $this->session_has_expired( $token ) ) {
			wc_add_notice( __( 'Your PayPal checkout session has expired. Please check out again.', 'pp-express-wc4jp' ), 'error' );
			return;
		}
		...
		try {
			...
			wc_add_notice( __( 'Sorry, an error occurred while trying to retrieve your information from PayPal. Please try again.', 'pp-express-wc4jp' ), 'error' );
	...
	public function maybe_render_cancel_link() {
		if ( $this->has_active_session() ) {
			printf(
				...
				esc_html__( 'Cancel', 'pp-express-wc4jp' )
			);
		}
	}
